...
const activateBtn = (e) => {
  ...
  // console.log(selcetedData, electedAttr);
  ...
  //     if(imagesAttr === selectedAttr || selectedAttr === 'all'){ // all을 클릭 : 선조건은 거짓이고 후조건은 참  -> 모든 이미지에 아래 코드 실행
  //                                                               // bag을 클릭 : 기존의 hide 클래스는 모두 삭제 -> selectedAttr과 imagesAttr이 같은 요소에 아래 코드 실행 
  //       image.classList.remove('hide')
  //      image.classList.add('show')
  //     } else { // selectedAttr 과 imagesAttr이 다른 요소에 hide 클래스 추가 -> 기존에 있던 show 클래스 삭제
  ...
  filterBtn.forEach((btn) => {
    btn.classList.remove('active') // 버튼 전체의 active 클래스 삭제

  targetBtn.classList.add('active') // 클릭한 버튼 요소에 active 추가
  })
}
